...
import { listAdminPeople, syncAdminPeopleList } from "@/lib/people";
...
export default function AdminAboutPage() {
  ...
  const load = useCallback(async () => {
      ...
      setError(null);
      try {
        const [pages, collections, boardPeople, teamPeople] = await Promise.all([
          listAdminCmsPages(token),
          listAdminCmsCollections(token),
          listAdminPeople(token, "board"),
      ...
      if (!partnersCol) {
        throw new Error("Partners collection missing. Seed CMS content.");
      }
      setPartnersId(partnersCol.id);
      setPartners(partnersCol.payload?.items ?? []);
      setBoard(boardPeople);
      setTeam(teamPeople);
      } catch (err) {
  ...
  async function savePeople(kind: PeopleTab, next: AboutPerson[]) {
    if (!token) return;
    const previous = kind === "board" ? board : team;
    setSaving(true);
    setMessage(null);
    setError(null);
    try {
      const synced = await syncAdminPeopleList(token, kind, next, previous);
      if (kind === "board") setBoard(synced);
      else setTeam(synced);
